...
const FIELD_MAPPINGS = {
    ...
    properties: {
        fields: ['id', 'ref', 'address', 'commission', 'letting_fee', 
            'user_id', 'owner_id', 'is_active' ],
    },
    tenants: {
        fields: ['id', 'ref', 'name', 'email', 'mobile', 'note', 'lease_term', 
            'lease_start_date', 'lease_end_date', 'rent', 'vacating_date', 
            'property_id', 'is_active'],
    },
    rentals: {
        fields: ['id', 'amount', 'created_at', 'payment_date',
...
function EditDataForm({ onUpdateData, type }) {
    ...
    const formik = useFormik({
        initialValues: {
            ...Object.fromEntries(fields.map(field => [field, generateInitialValue(field, dataToEdit[field])]))
        },
        ...
        onSubmit: (values) => {
            ...
            fields.forEach(field => {
                if (field === 'created_at' ||
                    ...
                    field === 'lease_start_date' ||
                    field === 'lease_end_date') {
                    formattedValues[field] = new Date(values[field]).toISOString().split('T')[0];
                }
            });
            fetch(ENDPOINTS[type] + id, {
                method: "PATCH",
            ...
            <form onSubmit={formik.handleSubmit}>
                {fields.map(field => (
                    <div>
                        ...
                        {formik.errors[field] ? <p className="errorsMessages">{formik.errors[field]}</p> : null}
                    </div>
                ))}              
